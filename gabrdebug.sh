...
return
    ...
    file)
        if ! [[ -v args  ]]; then
            ...
            _shiftArgs
            if [[ -v debug ]]; then
                echo "---SOURCING-$(_filename ${fn^^})---" >&2
            fi
            source $fn
            if [[ -v debug ]]; then
                echo "---READ-$(_filename $fn)---" >&2
            fi
            files+=([${fn}]=$fn)
            if [[ -v reuseFn ]] && [[ $(type -t "$reuseFn") = "function" ]]; then # only reuse the previous argument if the function is there
                ...
                #            Only when you write 'gabr example' will the file be imported AND the function called 
            fi
        fi
        ...
        ;;
    *)
        if [[ -v reuseFn ]] && [[ $reuseFn = $fn ]];  then
            if [[ -v debug ]]; then
                echo "$reuseFn is not a function" >&2
            fi
            _shiftArgs
            _onScope
            return
...
exitcode=$? # end onscope
if [[ -v debug ]]; then
    echo "---FAILED-$(_filename ${fn^^})---" >&2
    echo $(source $fn) >&2
else
    echo "Use 'gabr debug $@' to see detailed information about following error:"  >&2
fi
_error+=("$fn failed with exitcode $exitcode.")
fi # end onscope
# Test for initial scope of gabr function
if [[ "${funcstack[@]}" = "${FUNCNAME[@]}" ]]; then
    _onCleanUp
else
    if [[ -v debug ]]; then
        echo "Returning ${exitcode:-1}" >&2
